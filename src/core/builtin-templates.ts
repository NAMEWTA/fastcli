 ...
 * 真正的 `install` / `update` / `uninstall` 命令在加载 registry 时由本模块根据
 ...
 *   - update:    `volta install <pkg>@latest`
...
/**
 * 根据包管理器和 npm 包名，生成内置工具的 `install` / `update` / `uninstall`
 * 三个命令字符串。
 *
 ...
 * @returns          含 `install` / `update` / `uninstall` 三个字段的命令对象。
 *
 * @example
 *   buildBuiltinCommands('volta', '@anthropic-ai/claude-code')
 *   // => {
 *   //   install:   'volta install @anthropic-ai/claude-code',
 *   //   update:    'volta install @anthropic-ai/claude-code@latest',
